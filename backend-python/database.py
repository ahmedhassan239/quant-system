...
from config import DEFAULT_SYMBOLS, STABLECOIN_BLACKLIST, MOCK_TOKENS_BLACKLIST
...
# ──────────────────────────────────────────────────────────────────────
TOTAL_CAPITAL = 5000.0             # Total virtual capital
MAX_CONCURRENT_POSITIONS = 5        # Max symbols with open positions (BEAST MODE — aligned with MAX_GLOBAL_POSITIONS)
SLOT_BUDGET = 200.0                # Max USDT allocated per symbol
...
def get_active_symbols():
    ...
    try:
        ...
        symbol_list = [r.symbol for r in rows if r.symbol not in MOCK_TOKENS_BLACKLIST and r.symbol not in STABLECOIN_BLACKLIST]
...
def update_active_symbols(new_symbols: list[str]):
    ...
    try:
        ...
        # Exclude mock tokens and stablecoins
        new_symbols = [s for s in new_symbols if s not in MOCK_TOKENS_BLACKLIST and s not in STABLECOIN_BLACKLIST]
